...
const int btPin = 12; //Bt attach to digital 7

//pot for da lazer flashy
int potPin = A3;
int val = 0;
int daLazer = 2;
...
void loop() {
  ...
  if (digitalRead(btPin) == 0) {
   ...
   //Serial.println(" 'exit'");
  /** 
  POTENTIAL PROBLEM heh get it POT tential
  val = analogRead(potPin)/4;
  digitalWrite(daLazer, HIGH);
  delay(val);
  digitalWrite(daLazer, LOW);
  delay(val);
  /**
  **/
  /**
  ...
  **/  
  ...
  //I CHANGED IT TO INCHES
  distance = duration / 58.2 * .39;
